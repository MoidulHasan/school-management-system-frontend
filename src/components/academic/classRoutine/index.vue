<template>
    ...
    <div>
        <div class="col-12">
            <div class="card">
                <h5>Class Routine Data Table</h5>
                <DataTable :value="tableData" :paginator="true" class="p-datatable-gridlines" :rows="10" dataKey="id"
                    ...
                    :globalFilterFields="['class.name', 'subject.name', 'roomNumber.number', 'classTime.startTime', 'classTime.endTime', 'teacher.name']">
                    ...
                    <template #header>
                        <div class="flex justify-content-between flex-column sm:flex-row">
                            ...
                            <div>
                                <Button type="button" icon="pi pi-plus" label="Create Class Time Slot"
                    ...
                    <Column field="class" header="Class" filterField="data.class.name" style="min-width:12rem">
                        <template #body="{ data }">
                            {{ data.class.name }}
                        </template>
                    </Column>

                    <Column field="subject" header="Subject" filterField="data.subject.name" style="min-width:12rem">
                        <template #body="{ data }">
                            {{ data.subject.name }}
                        </template>
                    </Column>

                    <Column field="roomNumber" header="Room Number" filterField="data.roomNumber.number"
                        style="min-width:12rem">
                        <template #body="{ data }">
                            {{ data.roomNumber.number }}
                        </template>
                    </Column>

                    <Column field="startTime" header="Class Time" filterField="data.classTime.startTime"
                        style="min-width:12rem">
                        <template #body="{ data }">
                            {{ data.classTime.startTime + " - " + data.classTime.endTime }}
                        </template>
                    </Column>

                    <Column field="teacher" header="Teacher" filterField="data.teacher.name" style="min-width:12rem">
                        <template #body="{ data }">
                            {{ data.teacher.name }}
                        </template>
                    </Column>
    ...
    <Dialog header="Create Class Time Slot" :visible="showAddFormModal"
        ...
        <div class="grid grid-cols-1">
            <div class="p-5 w-1">
                <Dropdown v-model="addForm_SelectedTeacher" :options="teacherData" optionLabel="name" optionValue="name"
                    placeholder="Select a Teacher" />
                <p class="text-red-600" v-if="addForm_SelectTeacherError">{{ addForm_SelectTeacherError }}</p>
            </div>
        </div>

        <div class="grid grid-cols-2">
            <div class="p-5 w-1/2">
                <Dropdown v-model="addForm_SelectedClass" :options="classData" optionLabel="name" optionValue="name"
                    @change="fetchSubjectData" placeholder="Select a Class" />
                <p class="text-red-600" v-if="addForm_SelectClassError">{{ addForm_SelectClassError }}</p>
            </div>
            <div class="p-5 w-1/2">
                <Dropdown v-model="addForm_SelectedSubject" :options="subjectData" optionLabel="name" optionValue="name"
                    placeholder="Select a Subject" />
                <p class="text-red-600" v-if="addForm_SelectSubjectError">{{ addForm_SelectSubjectError }}</p>
            </div>
        </div>

        <div class="grid grid-cols-2">
            <div class="p-5 w-1/2">
                <Dropdown v-model="addForm_SelectedRoom" :options="roomNumberData" optionLabel="number"
                    optionValue="number" placeholder="Select a Room" />
                <p class="text-red-600" v-if="addForm_SelectRoomError">{{ addForm_SelectRoomError }}</p>
            </div>
            <div class="p-5 w-1/2">
                <Dropdown v-model="addForm_SelectedTime" :options="classTimeData" optionLabel="startTime, endTime"
                    optionValue="_id" placeholder="Select class time" />
                <!-- <template #body="{ data }">
                        {{ data.startTime + " - " + data.endTime }}
                    </template>
                </Dropdown> -->
                <p class="text-red-600" v-if="addForm_SelectTimeError">{{ addForm_SelectTimeError }}</p>
            </div>
        </div>
...
// Add form field
const addForm_SelectedClass = ref();
const addForm_SelectedSubject = ref();
const addForm_SelectedRoom = ref();
const addForm_SelectedTime = ref();
const addForm_SelectedTeacher = ref('');

// const classStartTimeError = ref();
// const classEndTimeError = ref();

const addForm_SelectClassError = ref('');
const addForm_SelectSubjectError = ref('');
const addForm_SelectRoomError = ref('');
const addForm_SelectTimeError = ref('');
const addForm_SelectTeacherError = ref('');
...
const fetchTableData = async () => {
    const fetchedData = await fetchData.get('academic/class-routine');
...
const fetchSubjectData = async () => {
    const subjectInfo = addForm_SelectedClass.value ? await fetchData.get(`academic/subject?className=${addForm_SelectedClass.value}`) : await fetchData.get(`academic/subject`);
...
const fetchAllClassRoom = async () => {
    ...
    if (classRoomInfo) {
        roomNumberData.value = classRoomInfo.data;
    }
}


const fetchTimeData = async () => {
    const fetchedData = await fetchData.get('academic/classtime');

    if (fetchedData) {
        classTimeData.value = fetchedData.data;
